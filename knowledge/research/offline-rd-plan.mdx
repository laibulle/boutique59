...
Nox30 has a first-stage shadow path behind
`GREYBOUND_NOX30_FIRST_STAGE_SHADOW_DESCRIPTOR`. The neural adapter runs beside
the analytic first stage and monitor telemetry reports its absolute error, but
the audio still uses the analytic stage. This is the first integration gate
before any replacement experiment.
