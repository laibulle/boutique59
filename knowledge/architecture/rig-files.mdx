...
```bash
target/release/greybound-cli --rig rigs/muffin-nox30.json5 --device 'DEVICE NAME' --input-channel 1 --output-channels 1,2 --sample-rate 44100 --period-size 16 --ir --monitor
```
...
- `cab.ir`: currently `tone3000-celestion`.
- `cab.bypassed`: whether the embedded speaker IR starts bypassed.
