...
Nox30 has an explicit first-stage shadow path, but it does not replace the
analytic audio path. Set `GREYBOUND_NOX30_FIRST_STAGE_SHADOW_DESCRIPTOR` to a
local `model.greybound.json` and the first common-cathode neural adapter will run
beside the analytic `first_stage`. Monitor telemetry then reports
`shadow first abs err avg/max` when the shadow is active. The sound still comes
from the analytic stage.
