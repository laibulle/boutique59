...
and pair it with the same Greybound cabinet IR. Use `Full Rig / Combo` only as a
fallback because it includes cab/mic coloration and will make model-stage
diagnosis less clean.
...
model. The Rust core now has an experimental reader/inference implementation for
that artifact shape. The next acceptance-relevant work is to add generated-vector
equivalence tests and compare the exported artifact against both SPICE holdouts
and Greybound's current analytic approximation.
...
This proves Python and Rust agree on the exported artifact. It does not prove
the model is a good tube replacement yet.
...
analysis against SPICE rather than immediate runtime integration.
