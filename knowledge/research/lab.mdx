...
log-spectral distance important, but also penalizes weak null residual,
envelope mismatch, and large gain correction. This is intentionally not an
objective tone score: if a control feels musically reactive, keep that listening
evidence. The score is only a guardrail against selecting a static NAM anchor
from one metric.
...
for several 1 kHz sine levels plus two-tone IMD cases, writes raw traces, packs
a `.npz`, and records hashes, node roles, train/validation/test splits,
component values, generated netlists, and operating point. Its purpose is to
make the SPICE-to-training contract executable before we expand to source/load
impedance sweeps, B+ perturbation, component tolerance sweeps, transient plucks,
and real DI windows.

Train the first experimental MLP artifact with:
...
This model is an export smoke test. It maps normalized `input_v` to normalized
`plate_ac_v` through a small static MLP and proves the PyTorch-to-Greybound
artifact path. It does not model tube memory, capacitance, source/load
interaction, B+ perturbation, or runtime Rust inference yet.
...
deterministic scalar MLP inference with `tanh` activations. This is still outside
the live audio path: no amp model uses it yet, and it has not been accepted as a
real-time tube-cell replacement.
...
`lab/reports/integrated-neural-first-stage.md`. This is the first full-chain gate
for a neural component: shadow telemetry gives local component error in volts,
and replace-vs-analytic audio metrics show the complete audible/runtime impact.
The current purpose is evidence collection, not promotion.
...
The first local report evaluates 7 stimuli and 6,566 decimated samples. The
aggregate RMSE is about `245 mV` against a zero-baseline RMSE around `644 mV`,
so the artifact learns useful shape. The hot held-out sine remains poor:
`sine_1khz_120mv` is about `583 mV` RMSE, or roughly `46%` relative RMSE. This
confirms the pipeline is working, but the current static MLP is not a sufficient
tube-cell replacement.
...
The current analytic baseline is substantially stronger than the first MLP:
about `80 mV` weighted RMSE versus about `245 mV` for the neural artifact. This
means the immediate research target is not runtime integration. The next useful
work is to understand why the analytic stage still has residual error and only
then decide whether a neural or fitted component improves the held-out cases.
